...
使用Komomo进行开发，那么我们就需要配置好开发环境和运行环境。

## 🏃‍♂️ 运行环境配置

我们在对`BDS`进行开发，所以首先我们需要一个安装好`LeviLamina`的`BDS`。

如果您还没有准备好一个`LeviLaminaBDS`环境，请前往[LeviLamina文档](https://lamina.levimc.org/zh/quickstart/)来完成安装。
...
?> 此处的`git submodule update --remote --recursive`用来从Github获取补全库。  
如果不进行这一步，tsc的编译将无法通过，您也会失去编写代码时的自动补全能力。
