...
Local<Value> BlockClass::getBlockEntityType() {
    try {
        if (!mBlock) return Local<Value>();
        // return ConvertToScriptX(mBlock->getBlockEntityType());
        return Local<Value>();
    }
    Catch;
}

Local<Value> BlockClass::getBlockTintType() {
    try {
        if (!mBlock) return Local<Value>();
        // return ConvertToScriptX(mBlock->getBlockTintType());
        return Local<Value>();
    }
    Catch;
}
...
// returns uchar instead of Brightness
// Local<Value> BlockClass::getLight() { CallFunction(Number, getLight().mValue) }

// Local<Value> BlockClass::getLightEmission() {
//     // CallFunction(Number, getLightEmission().mValue);
// }

// Local<Value> BlockClass::getMaterial() {
//     try {
//         if (!mBlock) return Local<Value>();
//         // auto &result = const_cast<Material &>(mBlock->getMaterial());
//         return MaterialClass::newMaterial(&result);
//     }
//     Catch;
// }
...
Local<Value> BlockClass::canProvideSupport(const Arguments &args) {
    CheckArgsCount(args, 2);
    CheckArgType(args[0], ValueKind::kNumber);
    CheckArgType(args[1], ValueKind::kNumber);
    try {
        if (!mBlock) return Local<Value>();
        auto face = static_cast<uchar>(args[0].asNumber().toInt32());
        auto type = ConvertFromScriptX<BlockSupportType>(args[1]);
        // return Boolean::newBoolean(mBlock->canProvideSupport(face, type));
        return Local<Value>();
    }
    Catch;
}
...
// optional_ref<class GetCollisionShapeInterface const>)  - not implemented
//     const;
Local<Value> BlockClass::clip(const Arguments &args) {
    ...
    CheckInstanceType(args[2], Vec3Class);
    CheckInstanceType(args[3], AABBClass);
    try {
        if (!mBlock) return Local<Value>();
        auto engine = EngineScope::currentEngine();
        auto pos = engine->getNativeInstance<BlockPosClass>(args[0])->mBlockPos;
...
Local<Value> BlockClass::copyState(const Arguments &args) {
    CheckArgsCount(args, 2);
    CheckInstanceType(args[0], BlockClass);
    CheckInstanceType(args[1], BlockStateClass);
    try {
        if (!mBlock) return Local<Value>();
        auto engine = EngineScope::currentEngine();
        auto fromBlock = engine->getNativeInstance<BlockClass>(args[0])->mBlock;
        auto state = engine->getNativeInstance<BlockStateClass>(args[1])->mBlockState;
        auto &result = const_cast<Block &>(mBlock->copyState(*fromBlock, *state));
...
Local<Value> BlockClass::destroy(const Arguments &args) {
    ...
    try {
        if (!mBlock) return Boolean::newBoolean(false);
        auto engine = EngineScope::currentEngine();
        auto region = engine->getNativeInstance<BlockSourceClass>(args[0])->mBlockSource;
        auto pos = engine->getNativeInstance<BlockPosClass>(args[1])->mBlockPos;
...
Local<Value> BlockClass::entityInside(const Arguments &args) {
    CheckArgsCountReturn(args, 3, Boolean::newBoolean(false));
    CheckInstanceTypeReturn(args[0], BlockSourceClass, Boolean::newBoolean(false));
    CheckInstanceTypeReturn(args[1], BlockPosClass, Boolean::newBoolean(false));
    CheckInstanceTypeReturn(args[2], ActorClass, Boolean::newBoolean(false));
    try {
        ...
        auto pos = engine->getNativeInstance<BlockPosClass>(args[1])->mBlockPos;
        auto entitySource = engine->getNativeInstance<ActorClass>(args[2])->mActor;
        // mBlock->entityInside(*region, pos, *entitySource);
        return Boolean::newBoolean(true);
    }
    CatchReturn(Boolean::newBoolean(false));
}
