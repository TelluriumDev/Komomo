...
public: /* Method */
    ...
    Local<Value> finalizeBlockCustomComponentEvents(const Arguments& args);
    ...
    Local<Value> keepState(const Arguments& args);
    ...
    Local<Value> waterSpreadCausesSpawn();
    Local<Value> buildSerializationId(const Arguments& args);
    Local<Value> setRuntimeId(const Arguments& args);
    Local<Value> _queueForTickBasedOnComponentConfiguration(const Arguments& args);
    Local<Value> _removeFromTickingQueues(const Arguments& args);
